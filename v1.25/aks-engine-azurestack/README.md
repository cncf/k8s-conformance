...
```bash
VERSION=v0.76.0
curl -L -o aks-engine-darwin.tar.gz https://github.com/Azure/aks-engine-azurestack/releases/download/${VERSION}/aks-engine-${VERSION}-darwin-amd64.tar.gz
mkdir ./aks-engine; tar xzf aks-engine-darwin.tar.gz -C ./aks-engine
```

Create a file called kubernetes1.25.json, the contents of the file should be the following cluster definition template:
