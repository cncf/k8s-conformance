# Mirantis Kubernetes Engine 3.9.0

The Mirantis Kubernetes Engine 3.9.0 platform is made up of a number of
components, including Kubernetes v1.34.

## Installing Mirantis Kubernetes Engine 3.9.0 to reproduce the results

You will need to deploy a cluster of atleast 3 nodes (minimum 1 manager and 2 worker) with Mirantis Container Runtime (MCR) v25.0.14 or newer installed. Mirantis recommends using [Launchpad](https://github.com/Mirantis/launchpad/) to ease the provisioning and installation of an MKE cluster.

### Provisioning a Cluster with Launchpad

The following section assumes that you have provisioned some instances with the necessary specifications defined in the [Reference Architecture Page](https://docs.mirantis.com/mke/3.8/common/mke-hw-reqs.html)

1. Download Launchpad. More details at [Launchpad Documentation Page](https://docs.mirantis.com/mke/3.8/launchpad.html)

    The latest available Launchpad binaries can be downloaded from here: <https://github.com/Mirantis/launchpad/releases>
...
3. Verify the installation by checking the installed tool version with the launchpad version command.
   ...
   ```shell
   version: 1.5.15
   commit: 41712d76ea9aed970dd764bffc33e062c9397816
   ```
   ...
   2. `--default-node-orchestrator=kubernetes` - This is to specify the default orchestrator as Kubernetes.
    ...
    spec:
      ...
      mke:
        ...
        imageRepo: registry.ci.mirantis.com/mirantiseng
        installFlags:
...
6. Once the above command is completed, you will see the Admin UI connection information, similar to the following example:
   ...
   INFO MKE cluster admin UI: https://<your-dns>.<region>.amazonaws.com/
...
2. Launch the conformance tests
    ...
    ```shell
    sonobuoy run --wait --mode=certified-conformance --kube-conformance-image-version=v1.34.3 --plugin-env='e2e.E2E_EXTRA_ARGS=--non-blocking-taints=com.docker.ucp.manager'
    ```
