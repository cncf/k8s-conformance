# Conformance tests for TaiChu v1.12.0
...
8. create directory ```v1.34/Taichu``` and copy  ```e2e.log, junit_01.xml``` to here.
```
mkdir -p ./v1.34/Taichu
cp ./sonobuoy/plugins/e2e/results/global/* ./v1.34/Taichu
```

9. Generate ```PRODUCT.yaml```
```
cat << EOF > ./v1.34/TaiChu/PRODUCT.yaml
