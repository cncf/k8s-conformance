...
4. Follow the wizard. Unless specified in the list below, go with the default or, if a selection/value is required but no default is provided, your own preference.
   - In step 2 ("Cluster"):
     - Choose "Control Plane Version" "1.34.5" (or newer).
...
When the cluster and all its nodes are shown with a green status indicater in the web interface, download the kubeconfig by clicking on the "Get Kubeconfig" button and select the option "OIDC via oidc-login plugin".
...
Additionally, if needed, you can reconnect to the running tests with:
