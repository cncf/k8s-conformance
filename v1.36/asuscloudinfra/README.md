...
This guide uses the following number of nodes for deployment, and the operating system uses Ubuntu 24.04 LTS for testing:

<span style="color:red">Please modify it according to the actual application.</span>

| Hostname   | IP Address    | Role   | OS              |
|------------|---------------|--------|-----------------|
| master01   | 10\.78\.26\.145 | Master | Ubuntu 24\.04\.4 |
| master02   | 10\.78\.26\.155 | Master | Ubuntu 24\.04\.1 |
| master03   | 10\.78\.26\.156 | Master | Ubuntu 24\.04\.1 |
| worker01   | 10\.78\.26\.144 | Node   | Ubuntu 24\.04\.4 |
| worker02   | 10\.78\.26\.187 | Node   | Ubuntu 24\.04\.4 |

In addition, all Master nodes provide an API Virtual IP **10.78.26.140:8443** through Keepalived for use.
...
$ sudo apt-get update
$ sudo apt-get install kubelet=1.36.4-1.1 kubeadm=1.36.4-1.1 kubectl=1.36.4-1.1
...
spec:
  ...
  containers:
  - name: kube-haproxy
    image: docker.io/library/haproxy:3.2.19-alpine
    resources:
      requests:
        cpu: 100m
    volumeMounts:
...
spec:
  ...
  containers:
  - name: kube-keepalived
    image: docker.io/osixia/keepalived:2.3.4
    env:
    ...
    - name: KEEPALIVED_UNICAST_PEERS
      value: "#PYTHON2BASH:['10.78.26.145', '10.78.26.155', '10.78.26.156']"
...
* KEEPALIVED_PRIORITY: Specifies the order of interfaces to take over when backup occurs. The smaller the number, the higher the priority. Here master01 is set to 150, master02 is 120, and master03 is 90.

#### 4. First control plane node

First create Kubeadm Init Configuration in <span style="color:red">master01</span>
...
controlPlaneEndpoint: 10.78.26.140:8443
...
You can now join any number of the control-plane node running the following command on each as root:
  ...
  kubeadm join 10.78.26.140:8443 --token <token> \
...
master01   NotReady   master   32s   v1.36.4
```

#### 5. Installing a Pod network add-on (Calico)

```sh
$ wget https://raw.githubusercontent.com/projectcalico/calico/v3.31.7/manifests/calico.yaml
...
NAME         STATUS   ROLES    AGE   VERSION
master01   Ready    master   10m   v1.36.4
master02   Ready    master   3m    v1.36.4
master03   Ready    master   74s   v1.36.4
```
...
```sh
$ kubeadm join 10.78.26.140:8443 \
    --token <token> \
    --discovery-token-ca-cert-hash <discovery-token-ca-cert-hash>
...
NAME           STATUS   ROLES    AGE     VERSION
master01     Ready    master   92s     v1.36.4
master02     Ready    master   3m33s   v1.36.4
master03     Ready    master   20m     v1.36.4
worker01     Ready    <none>   13m     v1.36.4
worker02     Ready    <none>   8m32s   v1.36.4
```

##### Test High Availability

Enter master01 to test the cluster HA function, here first shut down the node:

```sh
$ sudo poweroff
```

Then go to master02 and check whether the cluster can run normally through kubectl:

```sh
$ kubectl get no
NAME           STATUS   ROLES    AGE     VERSION
master01     NotReady master   92s     v1.36.4
master02     Ready    master   3m33s   v1.36.4
master03     Ready    master   20m     v1.36.4
worker01     Ready    <none>   13m     v1.36.4
worker02     Ready    <none>   8m32s   v1.36.4
