...
- Linux Host OS (preferably Ubuntu 25.10)
- Vagrant: 2.4.9+
- VirtualBox 7.2+
...
```shell
wget https://raw.githubusercontent.com/darxkies/k8s-tew/2.4.14/Vagrantfile
...
```shell
wget https://github.com/darxkies/k8s-tew/releases/download/2.4.14/k8s-tew
chmod u+x k8s-tew
```
...
```shell
./k8s-tew initialize
./k8s-tew configure --version-k8s=v1.36.0
./k8s-tew configure --version-kube-apiserver=registry.k8s.io/kube-apiserver:v1.36.0
./k8s-tew configure --version-kube-controller-manager=registry.k8s.io/kube-controller-manager:v1.36.0
./k8s-tew configure --version-kube-proxy=registry.k8s.io/kube-proxy:v1.36.0
./k8s-tew configure --version-kube-scheduler=registry.k8s.io/kube-scheduler:v1.36.0
...
./sonobuoy run --mode=certified-conformance --plugin-env=e2e.E2E_EXTRA_ARGS="--ginkgo.v" --kubernetes-version=v1.36.0
```
