...
```bash
minikube start --kubernetes-version 1.36.2 --nodes=2
```
...
Create a standalone virtual cluster using `vcluster version 0.35.0` (latest) using
the following command:

```bash
# Create the vCluster
sudo curl -sfL https://github.com/loft-sh/vcluster/releases/download/v0.35.0/install-standalone.sh | sh -s -- --vcluster-name standalone --config vcluster.yaml
```
