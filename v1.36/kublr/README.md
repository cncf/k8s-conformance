...
Linux:
    ...
    sudo docker run --name kublr -d --restart=unless-stopped -p 9080:9080 kublr/kublr:1.30.0

Windows:

    docker run --name kublr -d --restart=unless-stopped -p 9080:9080 kublr/kublr:1.30.0
...
Open a terminal and launch following command:
    ...
    sudo docker run --name kublr -d --restart=unless-stopped -p HOST_PORT:9080 -e KUBLR_HOST=HOST_IP:HOST_PORT kublr/kublr:1.30.0
...
1. Set:
    * **Region**
    * **Kublr Agent** (select 1.34.x-beta agent build to install k8s 1.34)
