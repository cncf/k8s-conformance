...
Linux:
    ...
    sudo docker run --name kublr -d --restart=unless-stopped -p 9080:9080 kublr/kublr:1.17.0

Windows:

    docker run --name kublr -d --restart=unless-stopped -p 9080:9080 kublr/kublr:1.17.0
...
Open a terminal and launch following command:
    ...
    sudo docker run --name kublr -d --restart=unless-stopped -p HOST_PORT:9080 -e KUBLR_HOST=HOST_IP:HOST_PORT kublr/kublr:{{< param kublrVersion >}}
...
Here's a Kublr Demo/Installer launch command example:
    ...
    sudo docker run --name kublr -d --restart=unless-stopped -p 9080:9080 kublr/kublr:{{< param kublrVersion >}}
