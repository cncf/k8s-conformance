...
6. Once image download completes successfully, use the UI cluster create wizard
   to fill up all the necessary information:
     ...
     d) Select 1.22.x as the K8s version to be deployed.
     e) Select the storage container.
     f) Initiate cluster creation.
     g) UI will display the status of deployment.

Next, follow the steps to install a fresh cluster, choosing `v1.22` as the Kubernetes
...
This would build the latest Sonobuoy release [v0.56.4](https://github.com/vmware-tanzu/sonobuoy/releases/tag/v0.56.4).
