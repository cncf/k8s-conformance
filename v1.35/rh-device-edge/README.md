...
1. Prepare two RHEL 9.8 hosts suitable for running MicroShift. The following configuration is recommended:
- RHEL 9.8
- 2 CPU cores
- 4GB of RAM
- 20 GB of storage
...
sudo tee "/etc/yum.repos.d/rhocp-4.21-for-rhel-9-mirrorbeta-x86_64-rpms.repo" >/dev/null <<EOF
[rhocp-4.21-for-rhel-9-mirrorbeta-x86_64-rpms]
name=Beta rhocp-4.22 RPMs for RHEL 9
baseurl=https://mirror.openshift.com/pub/openshift-v4/x86_64/dependencies/rpms/4.22-el9-beta/
enabled=1
gpgcheck=0
...
[microshift-preview]
name=Beta microshift-4.22 RPMs for RHEL 9
...
# need to run configure-node and not many more things.
```
PRI_HOST=microshift-pri
PRI_ADDR=192.168.124.229
SEC_HOST=microshift-sec
SEC_ADDR=192.168.124.124
USER=redhat
```
...
```
git clone https://github.com/openshift/microshift.git --branch release-4.22
cd ./microshift/
scp -o StrictHostKeyChecking=no ./scripts/multinode/configure-node.sh ${USER}@${PRI_ADDR}:
ssh -t ${USER}@${PRI_ADDR} "./configure-node.sh"
```

4. Copy the kubeconfig generated in the primary to the secondary host by running the following command.

```
scp -o StrictHostKeyChecking=no \
    ${USER}@${PRI_ADDR}:kubeconfig-bootstrap \
    ${USER}@${SEC_ADDR}:kubeconfig-bootstrap
```

5. Run the following commands to copy the configuration script to the secondary host and run it remotely.

```
scp -o StrictHostKeyChecking=no ./scripts/multinode/configure-node.sh ${USER}@${SEC_ADDR}:
ssh -t ${USER}@${SEC_ADDR} "./configure-node.sh --bootstrap-kubeconfig kubeconfig-bootstrap"
```
...
scp ${USER}@${PRI_ADDR}:kubeconfig-bootstrap ${KUBECONFIG}
```
