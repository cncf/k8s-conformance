...
ionosctl k8s cluster create --name k8s-conformance-1.35 --k8s-version 1.35.6 --wait

# Create a Data Center for your Nodes and notice the datacenter-id
ionosctl datacenter create --name k8s-conformance-1.35-dc --location de/txl --wait

# Create a Node Pool
ionosctl k8s nodepool create --datacenter-id <datacenter-id> \ 
  --name k8s-conformance-1.35-nodepool  --cluster-id <cluster-id> \ 
  --node-count 3 --k8s-version 1.35.6 --wait
