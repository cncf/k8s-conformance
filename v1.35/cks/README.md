...
### UI
To create a cluster via the UI, sign into [console.coreweave.com/login](https://console.coreweave.com/login). On the Clusters view, click `Create Cluster` on the top right, type a name, select `v1.35`, (keep exposed to public internet enabled), hit next, select the desired zone, use default VPC, and then click next through the rest and submit. You can generate a kubeconfig with this cluster in the `Tokens` menu, by creating a token and specifying your cluster when downloading the kubeconfig.
...
In v1.35, [in-place pod resizing is a stable feature](https://kubernetes.io/blog/2025/12/19/kubernetes-v1-35-in-place-pod-resize-ga/). Conformant clusters are expected to support this, however this feature [requires kubelets to be configured with the default, `none`, CPU Management Policy](https://kubernetes.io/docs/tasks/configure-pod-container/resize-container-resources/#:~:text=Node%20Policies%3A%20Pods%20managed%20by%20static%20CPU%20or%20Memory%20manager%20policies%20cannot%20be%20resized%20in%2Dplace.). CKS Clusters use `static` by default, so for a conformant cluster, you will need to update this setting before proceeding:

Request payload `update-kubelet-cpumanagerpolicy.json`:
```
{
      "id": "<your cluster id>",
      "updateMask": "kubelet",
...
Update command:
```
curl -X PATCH https://api.coreweave.com/v1beta1/cks/clusters/<your cluster id here> -H "Content-Type: application/json" -H "Authorization: Bearer $API_ACCESS_TOKEN" -i -d @update-kubelet-cpumanagerpolicy.json
```
