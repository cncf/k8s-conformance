# Conformance tests for Elastisys Welkin@ Cluster API v0.10.0 (based on Kubernetes v1.35.6)
...
1. Create configuration and then modify according to your needs:
    ...
    - `fingerprint` is the PGP fingerprint used by SOPS for secrets encryption within the `CK8S_CONFIG_PATH`.
...
1. Bootstrap the management cluster:
    ...
    ```sh
    ./bin/ck8s-capi bootstrap $(yq '.managementCluster' "${CK8S_CONFIG_PATH}/capi/values.yaml")
    ```
...
You should also have a `kubeconfig`s at `${CK8S_CONFIG_PATH}/.state/kube_config_<cluster-name>.yaml` that you can use to access the clusters.
