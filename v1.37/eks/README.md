...
```bash
eksctl create cluster --version=1.37 \
  --name=sample-cluster-1.37 \
```
...
if [[ "$(uname)" == "Darwin" ]]
then
  SONOBUOY=https://github.com/vmware-tanzu/sonobuoy/releases/download/v0.57.5/sonobuoy_0.57.1_darwin_amd64.tar.gz
else
  SONOBUOY=https://github.com/vmware-tanzu/sonobuoy/releases/download/v0.57.5/sonobuoy_0.57.1_linux_386.tar.gz
fi
...
--sonobuoy-image=sonobuoy/sonobuoy:v0.57.5
````
...
## Cleanup
```bash
eksctl delete cluster --name=sample-cluster-1.37
```
