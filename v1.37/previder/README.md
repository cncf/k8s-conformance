...
2. Choose a cluster name e.g. "Conformance 1.37"
2. Disable the automatic updates option and select Kubernetes version v1.37.x.
...
```bash
./sonobuoy run --mode=certified-conformance --wait --kubeconfig ~/Downloads/Conformance\ 1.37-kubeconfig
```
...
```bash
outfile=$(./sonobuoy retrieve --kubeconfig ~/Downloads/Conformance\ 1.37-kubeconfig)
