...
1. Download and extract `openshift-install` and `oc` binaries from https://mirror.openshift.com/pub/openshift-v4/clients/ocp-dev-preview/4.20.0-ec.5/.
...
2. Follow the [test instructions](https://github.com/cncf/k8s-conformance/blob/master/instructions.md#hydrophone)
   to run the conformance tests. OpenShift cluster disables [scheduling on control plane nodes](https://docs.openshift.com/container-platform/4.19/nodes/nodes/nodes-nodes-managing.html#nodes-nodes-working-master-schedulable_nodes-nodes-managing)
...
```
hydrophone --conformance --conformance-image registry.k8s.io/conformance:v1.33.2 --extra-args="--allowed-not-ready-nodes=3"
```
