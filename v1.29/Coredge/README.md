...
```bash
./master-installer.sh "v1.29.0" "<master-node-ip>"
```
...
```
sonobuoy run --plugin-env=e2e.E2E_EXTRA_ARGS="--non-blocking-taints=node-role.kubernetes.io/control-plane --ginkgo.v" --mode=certified-conformance --kubernetes-version="v1.29.0"
```
